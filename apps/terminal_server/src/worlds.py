...
# ---------- Chatbot demo ---------- #
class RenderBotChatOnboardWorld(OnboardWorld):
    """
    ...
    def generate_world(opt, agents):
        return RenderBotChatOnboardWorld(opt=opt, agent=agents[0])

    def parley(self):
        self.episodeDone = True


class RenderBotChatTaskWorld(World):
    """
    ...
    def generate_world(opt, agents):
        ...
        return RenderBotChatTaskWorld(
            opt,
            agents[0],
            create_agent_from_shared(
                opt['shared_bot_params'][RenderBotChatTaskWorld.MODEL_KEY]
            ),
        )
...
class RenderOverworld(World):
    ...
    @staticmethod
    def generate_world(opt, agents):
        return RenderOverworld(opt, agents[0])
